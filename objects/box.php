...
class Box extends Config {
	...
	function add () {
		$query = "INSERT INTO
					" . $this->table_name . "
				SET
					title = ?, location = ?, rows = ?, cols = ?, squares = ?, place_id = ?";
		...
		$stmt->bindParam(6, $this->place_id);
...
//			$this->countBooks($row['id']);
	...
	function readAll () {
		...
		$stmt = $this->conn->prepare($query);
		$stmt->execute();
		$this->boxesList = $this->placesID = $this->boxesName = array();
		while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
			...
			if ($row['id']) {
				...
				$this->boxesList[] = $row;
				if ($row['place_id']) {
					$this->boxesName[] = $row['title'];
					$this->placesID[] = $row['place_id'];
				}
			}
		}
	...
	function edit () {
		...
		$stmt->bindParam(4, $this->place_id);
...
*/		$outputFile = $file;
	...
	function search () {
		$query = "SELECT id,box_id,square_id,price,price_rent,num,cover FROM ".$this->table_name."_books WHERE link = ?";
		$stmt = $this->conn->prepare($query);
		$stmt->bindParam(1, $this->keyword);
		$stmt->execute();
		$data = array();
		$this->placesID = $this->boxesName = array();
		while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
			$row['boxInfo'] = $boxInfo = $this->getBoxInfo($row['box_id']);
			$this->boxesName[] = $boxInfo['title'];
			$this->placesID[] = $boxInfo['place_id'];
			$this->placesName[] = $boxInfo['location'];
			$row['link'] = $boxInfo['link'].'?b='.$row['id'];
			$data[] = $row;
		}
		return $data;
	}
...
*/
	...
	function changeHandlingRequest_ToDone ($square_id = null) {
		if (!$square_id) $square_id = $this->book_square_id;
/*		$query = "UPDATE
					" . $this->table_name . "_books_buy
				SET done = 1 
				WHERE box_id = ? 
...
*/		$query = "UPDATE
					" . $this->table_name . "_books_buy
				SET done = 1 
				WHERE box_id = ? AND square_id = ? ";
		$stmt = $this->conn->prepare($query);

		// bind values
		$stmt->bindParam(1, $this->id);
		$stmt->bindParam(2, $square_id);
...
//			$updatePrevious = $this->changeHandlingRequest_ToDone(); // update status of my request previous request
			// handle my request
...
//		return false; // not my turn
	...
	function getBooksDetails ($iid = null) {
		if (!$iid) $iid = $this->book_id;
		$query = "SELECT num,price,id FROM ".$this->table_name."_books WHERE id = ? ORDER BY id DESC LIMIT 0,1";
		$stmt = $this->conn->prepare($query);
		$stmt->bindParam(1, $iid);
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		if ($stmt->rowCount() > 0) {
			$this->book_num = (int)$row['num'];
			$this->book_price = (int)$row['price'];
			return true;
		}
		return false;
	}
	
	function getRequestDetails ($square_id = null) {
		if (!$square_id) $square_id = $this->book_square_id;
		$query = "SELECT * FROM ".$this->table_name."_books_buy WHERE done = 0 AND box_id = ? AND square_id = ? ORDER BY id ASC LIMIT 0,1";
		$stmt = $this->conn->prepare($query);
		$stmt->bindParam(1, $this->id);
		$stmt->bindParam(2, $square_id);
		$stmt->execute();
		$row = $stmt->fetch(PDO::FETCH_ASSOC);
		if ($stmt->rowCount() > 0) {
			$this->book_id = $row['iid'];
			$this->getBooksDetails($row['iid']); // set $this->book_num and $this->book_price inside this function
		}
		return ($stmt->rowCount() > 0) ? $row : null;
	}
	
	function arduinoBuy_finishGet () { // handle everything here
		$square_id = (int)file_get_contents(MAIN_PATH.'/data/box/'.$this->id.'.json');
		// update file
		file_put_contents(MAIN_PATH.'/data/box/'.$this->id.'.json', 0);
		// get u request
		$requestDetails = $this->getRequestDetails($square_id); // set $this->book_id and $this->book_num and $this->book_price inside this function
		if ($requestDetails) {
			$uRequest = $requestDetails['uid'];
			// substract coins
			$price = 0-$this->book_price;
			$this->addCoin($price, $uRequest);
			// change status of buy request of this square_id
			$this->changeHandlingRequest_ToDone($square_id);
			// decrease book number
			$this->decreaseBookNum();
			$valAr = array(
						'type' => 'finish-buy',
						...
						'iid' => $square_id,
						'post_id' => $this->id,
						'content' => json_encode(array('box_title' => $this->title), JSON_UNESCAPED_UNICODE)
					);
			$this->addNoti($valAr, $uRequest);
		}
	}

	function getRequestFromFile () {
/*		if ($this->checkMyRequestDone()) {
			echo 0;
		} else echo {
*/			return file_get_contents(MAIN_PATH.'/data/box/'.$this->id.'.json');
//		}
	}

	function checkMyRequestDone () {
		$query = "SELECT id,done,square_id FROM
					" . $this->table_name . "_books_buy
				WHERE box_id = ? AND square_id = ?
				ORDER BY id DESC
